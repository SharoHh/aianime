...
function titleGenreList(item = {}){
  return Array.isArray(item?.genres) ? item.genres.map(genre => cleanPublicText(genre)).filter(Boolean) : []
}

function titleGenreText(item = {}){
  const genres = titleGenreList(item).slice(0, 4)
  return genres.length ? genres.join(', ') : 'аниме'
}

function titleKindText(item = {}){
  return isMovieAnime(item) || item?.kind === 'movie' ? 'полнометражное аниме' : 'аниме-сериал'
}

function titleAudienceText(item = {}, title = 'Этот тайтл'){
  const genres = titleGenreText(item).toLowerCase()
  const yearText = item?.year ? ` ${item.year} года` : ''
  if(/роман|драма|повседнев|комед/.test(genres)) return `${title} подойдёт зрителям, которые выбирают аниме про чувства, настроение и живых персонажей${yearText}.`
  if(/экшен|боев|приключ|фэнтези|сёнен|спорт/.test(genres)) return `${title} подойдёт тем, кто ищет динамичное аниме${yearText}: с движением, конфликтами и понятной мотивацией героев.`
  if(/детектив|триллер|психолог|мистик|ужас/.test(genres)) return `${title} стоит открыть, если хочется более напряжённого просмотра с загадками и вниманием к деталям.`
  return `${title} подойдёт, если хочется выбрать аниме по атмосфере, жанрам и описанию, не тратя время на случайный поиск.`
}

function titleWatchReasonText(item = {}, title = 'Тайтл'){
  const kind = titleKindText(item)
  const genres = titleGenreText(item)
  const episodes = Number(item?.episodes || 0)
  const episodeText = episodes > 1 ? `, ${episodes} серий` : episodes === 1 ? ', 1 серия' : ''
  const yearText = item?.year ? ` ${item.year} года` : ''
  return `${title} — ${kind}${yearText} в жанрах ${genres}${episodeText}. На странице собраны описание, рейтинг, серии, плеер, похожие тайтлы и комментарии, чтобы быстро понять, стоит ли начинать просмотр.`
}

function buildTitleFaqEntries(item = {}, title = 'Тайтл', displayEpisodeCount = ''){
  const kind = titleKindText(item)
  const genres = titleGenreText(item)
  const yearText = item?.year ? ` ${item.year} года` : ''
  const episodeCount = Number(displayEpisodeCount || 0)
  const episodeAnswer = episodeCount > 1
    ? `В карточке указано ${episodeCount} серий. Если у тайтла есть плеер, серии доступны в блоке просмотра ниже.`
    : episodeCount === 1
      ? 'В карточке указан формат на одну серию или фильм. Смотреть можно через блок плеера на этой странице.'
      : 'Точное количество серий уточняется по данным каталога и плеера. Актуальный список отображается в блоке просмотра.'

  return [
    {
      question:`О чём аниме «${title}»?`,
      answer:`«${title}» — ${kind}${yearText}. Перед просмотром можно оценить описание, жанры, рейтинг и похожие рекомендации на AIanime.`
    },
    {
    ...
    {
      question:`Какие жанры у «${title}»?`,
      answer:`Основные жанры: ${genres}. По ним можно перейти к похожим аниме в каталоге.`
    },
    {
      question:`Кому подойдёт «${title}»?`,
      answer:titleAudienceText(item, title)
    }
  ]
}

function buildFaqJsonLd(entries = []){
  const list = Array.isArray(entries) ? entries.filter(entry => entry?.question && entry?.answer) : []
...
export default async function AnimePage({ params, searchParams }){
  ...
  const info = visibleInfoRows([
    ...
    ['Количество серий', displayEpisodeCount],
