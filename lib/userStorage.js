  ...
  runQuietly(async () => {
    ...
    if(!cleanStatus){
      const { error } = await supabase
        ...
        .eq('anime_slug', item.slug)
      if(error){
        if(isMissingRelation(error, 'user_anime_library')) return { ok:true, localOnly:true, message:friendlyLibrarySchemaMessage() }
        throw error
      }
      return { ok:true }
    }
    ...
    const { error } = await supabase
      .from('user_anime_library')
      .upsert(normalizeLibrary(user.id, item, cleanStatus), { onConflict:'user_id,anime_slug' })
    if(error){
      if(isMissingRelation(error, 'user_anime_library')) return { ok:true, localOnly:true, message:friendlyLibrarySchemaMessage() }
      throw error
    }
...
export async function syncLocalAnimeDataToCloud(){
  ...
  if(library.length){
    const { error } = await supabase.from('user_anime_library').upsert(library, { onConflict:'user_id,anime_slug' })
    if(error && !isMissingRelation(error, 'user_anime_library')) throw error
  }

  if(historyLocalOnly){
    setUserSyncStatus({ state:'idle', message:historyLocalOnlyMessage || friendlyHistorySchemaMessage() })
    return { ok:true, localOnly:true, message:historyLocalOnlyMessage || friendlyHistorySchemaMessage(), counts:{ favorites:favorites.length, history:history.length, ratings:ratings.length, library:library.length } }
  }

  setUserSyncStatus({ state:'ok', message:'Данные аккаунта синхронизированы' })
  return { ok:true, counts:{ favorites:favorites.length, history:history.length, ratings:ratings.length, library:library.length } }
}
...
export async function restoreCloudAnimeData(options = {}){
  ...
  let libraryResult = { data:[], error:null, localOnly:false }
  try{
    libraryResult = await supabase
      .from('user_anime_library')
      .select('anime_slug,status,title,poster,rating,meta,episode,voice,updated_at')
      .eq('user_id', user.id)
      .order('updated_at', { ascending:false })
      .limit(600)
    if(libraryResult.error && isMissingRelation(libraryResult.error, 'user_anime_library')){
      libraryResult = { data:[], error:null, localOnly:true, message:friendlyLibrarySchemaMessage() }
    }else if(libraryResult.error){
      throw libraryResult.error
    }
  }catch(error){
    if(isMissingRelation(error, 'user_anime_library')) libraryResult = { data:[], error:null, localOnly:true, message:friendlyLibrarySchemaMessage() }
    else throw error
  }
  ...
  if(!silent){
    if(historyResult?.localOnly || syncResult?.localOnly){
      setUserSyncStatus({ state:'idle', message:historyResult?.message || libraryResult?.message || syncResult?.message || friendlyHistorySchemaMessage() })
    }else{
      setUserSyncStatus({ state:'ok', message:'Данные аккаунта обновлены' })
    }
  }
  return { ok:true, localOnly:Boolean(historyResult?.localOnly || libraryResult?.localOnly || syncResult?.localOnly), sync:syncResult }
}
...
export async function clearCloudAnimeData(storageKey = 'all'){
  ...
  if(storageKey === 'all' || storageKey === 'anime:library'){
    deletions.push(supabase.from('user_anime_library').delete().eq('user_id', user.id))
  }

  const results = await Promise.all(deletions)
  const failed = results.find(result => result?.error && !isMissingRelation(result.error, 'user_anime_library'))
