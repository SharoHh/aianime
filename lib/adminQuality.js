...
export function isBadPosterUrl(value){
  ...
  if(text.includes('no-poster')) return true
  if(text.includes('default-poster')) return true
  if(text.includes('/posters/magic')) return true
  if(/\/posters\/[^?#]+\.svg(?:[?#].*)?$/i.test(text)) return true
  if(/\/banners\/[^?#]+\.svg(?:[?#].*)?$/i.test(text)) return true
  if(text.includes('%2fposters%2f') && text.includes('.svg')) return true
  if(text.includes('%2fbanners%2f') && text.includes('.svg')) return true
  return false
}
