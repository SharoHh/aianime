// AIanime v152: compact visible SEO content for genre, studio, collections and year pages.
// Pure text/schema helpers only: no runtime data fetching and no UI components.
...
export function faqJsonLd(faq = []){
  const items = (Array.isArray(faq) ? faq : [])
    .map(item => ({ question:safeText(item.question), answer:safeText(item.answer) }))
...
export function webPageJsonLd({ name, description, path = '/' } = {}){
  return {
    '@context':'https://schema.org',
    '@type':'WebPage',
    name:safeText(name),
...
const GENRE_DESCRIPTIONS = {
  ...
  'сёнен':'Сёнен-аниме обычно сочетает развитие героя, дружбу, соперничество и яркие конфликты. Это один из самых удобных жанров для старта, если хочется динамичного сериала с понятной целью и сильными персонажами.',
  ...
  'сэйнэн':'Сэйнэн чаще рассчитан на более взрослое восприятие: здесь могут быть сложнее темы, спокойнее темп и больше внимания к последствиям решений. Жанр хорошо подходит для зрителей, которым хочется зрелой истории.',
...
export function buildGenreSeoContent(title, items = []){
  ...
  const faq = [
    { question:`Что посмотреть в жанре ${genre}?`, answer:`Начни с верхних карточек: они отсортированы по рейтингу и популярности внутри жанра ${genre}.${exampleText}` },
    { question:`Кому подойдут аниме в жанре ${genre}?`, answer:`Такие тайтлы подойдут зрителям, которым важны характерные темы жанра, узнаваемый вайб и быстрый переход к просмотру на русском.` },
    { question:`Сколько аниме в жанре ${genre} есть на AIanime?`, answer:`Сейчас на странице собрано ${count} тайтлов в этом жанре. Каталог может обновляться после синхронизации с источниками и расписанием.` }
  ]

  return {
    summary:`Как выбирать аниме в жанре ${genre}`,
    intro,
...
export function buildStudioSeoContent(studio, items = []){
  ...
  const faq = [
    { question:`Какие аниме выпустила студия ${name}?`, answer:`На этой странице собраны тайтлы студии ${name}, доступные в каталоге AIanime.${examples.length ? ` Среди них: ${examples.join(', ')}.` : ''}` },
    { question:`Сколько тайтлов студии ${name} есть на AIanime?`, answer:`Сейчас в каталоге показано ${count} тайтлов этой студии. Список может обновляться после синхронизации каталога.` },
    { question:`Как выбрать аниме студии ${name}?`, answer:`Ориентируйся на рейтинг, жанры, год выхода и статус. После открытия тайтла можно перейти к похожим аниме и плееру.` }
  ]

  return {
    summary:`Что смотреть у студии ${name}`,
...
export const COLLECTION_SEO = [
  { title:'Если грустно', q:'эмоциональное тёплое аниме', description:'Тёплые и эмоциональные аниме для вечера, когда хочется мягкой истории, поддержки, романтики или спокойной драмы без лишнего шума.', filter:item => (item.genres || []).some(g => ['Драма','Романтика','Повседневность','Ияшикэй'].includes(g)) },
  { title:'Хочу приключений', q:'приключения и экшен', description:'Динамичные тайтлы с путешествиями, экшеном, фэнтези и ощущением большой цели. Хороший выбор, когда хочется движения и ярких событий.', filter:item => (item.genres || []).some(g => ['Приключения','Экшен','Фэнтези','Исэкай'].includes(g)) },
  { title:'Для отдыха', q:'лёгкое аниме на вечер', description:'Лёгкие аниме на вечер: комедия, повседневность, романтика и короткие сезоны, которые удобно смотреть без тяжёлого настроения.', filter:item => (item.genres || []).some(g => ['Комедия','Повседневность','Романтика','Ияшикэй'].includes(g)) || Number(item.episodes || 0) <= 13 },
  { title:'Что-то новенькое', q:'популярные новинки', description:'Свежие и недавние тайтлы из каталога AIanime. Подборка подойдёт, если хочется посмотреть новые аниме последних сезонов.', filter:item => Number(item.year || 0) >= 2024 },
  { title:'Шедевры', q:'лучшие аниме по рейтингу', description:'Сильные аниме с высоким рейтингом и заметной репутацией. Удобная точка входа для тех, кто хочет начать с проверенных тайтлов.', filter:item => Number(item.rating || item.score || 0) >= 8.6 }
]

export function collectionFaq(){
  return [
    { question:'Как выбрать аниме по настроению?', answer:'Открой подходящую подборку: для отдыха, грустного настроения, приключений, новинок или рейтинговых шедевров. Внутри каждой группы показаны тайтлы из каталога AIanime.' },
    { question:'Подборки на AIanime обновляются?', answer:'Да, подборки строятся на данных каталога: жанрах, годе выхода, рейтинге и доступных тайтлах. После обновления каталога состав может меняться.' },
    { question:'Можно ли получить персональную подборку?', answer:'Да, у каждой подборки есть переход в AI-подбор. Можно описать настроение или любимые тайтлы и получить более точные рекомендации.' }
  ]
}
...
export function buildYearSeoContent(year, items = []){
  ...
  const faq = [
    { question:`Какие аниме ${safeYear} года посмотреть?`, answer:`Начни с верхних карточек на странице: там собраны тайтлы ${safeYear} года из каталога AIanime, отсортированные по рейтингу и популярности.${top.length ? ` Среди заметных вариантов: ${top.join(', ')}.` : ''}` },
    { question:`Сколько аниме ${safeYear} года есть на AIanime?`, answer:`Сейчас на странице показано ${count} тайтлов ${safeYear} года${ongoing ? `, включая ${ongoing} онгоингов` : ''}.` },
    { question:`Как выбрать новое аниме ${safeYear} года?`, answer:`Смотри на жанры, рейтинг, статус и количество серий. Для быстрого выбора также можно перейти в похожие тайтлы на странице конкретного аниме.` }
  ]

  return {
    summary:`Аниме ${safeYear} года: как выбрать`,
    ...
    points:[
      `Сравнивай тайтлы по рейтингу, жанрам, статусу и количеству серий.`,
      `Открывай страницу тайтла, чтобы посмотреть описание, плеер и похожие аниме.`,
      `Возвращайся к странице после обновлений каталога: список может пополняться.`
    ],
    faq
  }
}
