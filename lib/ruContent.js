const GENRE_MAP = new Map(Object.entries({
  ...
  'Love Status Quo':'Романтический статус-кво',
  'Urban fantasy':'Городское фэнтези',
  'Love status quo':'Романтический статус-кво'
}))
...
const TECHNICAL_SYMBOL_RE = /[●◦○◆■▪▫◼◾⬤]+/g

export function cleanPublicText(value){
  const text = clean(value)
  if(!text) return ''
  return text
    .replace(TECHNICAL_SYMBOL_RE, '')
    .replace(/\s+([,.:;!?])/g, '$1')
    .replace(/([:;,.])\s*([:;,.])/g, '$1')
    .replace(/\s{2,}/g, ' ')
    .replace(/[:;,.-]\s*$/g, '')
...
export function isPlaceholderText(value){
  const text = cleanPublicText(value).toLowerCase()
  if(!text) return true
  return [
    '—',
    '-',
...
export function makeRussianDescription(item = {}){
  const title = cleanPublicText(item.title_ru || item.displayTitle || item.title || item.original_title || item.originalTitle || 'Этот тайтл')
  const original = cleanPublicText(item.title_orig_kodik || item.original_title || item.originalTitle || '')
...
export function preferRussianDescription(item = {}){
  const ru = cleanPublicText(item.description_ru || item.descriptionRu)
