...
const buildVkAuthEnabled = process.env.NEXT_PUBLIC_VK_AUTH_ENABLED === '1'
const buildVkAuthProvider = process.env.NEXT_PUBLIC_VK_AUTH_PROVIDER || 'custom:vk'
...
function cleanConfig(config){
  ...
  const rawVkEnabled = config?.vkAuthEnabled
  const vkAuthEnabled = rawVkEnabled === true || rawVkEnabled === 1 || String(rawVkEnabled || '') === '1'
  const vkAuthProvider = String(config?.vkAuthProvider || buildVkAuthProvider || 'custom:vk').trim() || 'custom:vk'
  return { supabaseUrl, supabaseAnonKey, vkAuthEnabled, vkAuthProvider }
}
...
export function getBuildSupabaseConfig(){
  return cleanConfig({
    supabaseUrl: buildSupabaseUrl,
    supabaseAnonKey: buildSupabaseAnonKey,
    vkAuthEnabled: buildVkAuthEnabled,
    vkAuthProvider: buildVkAuthProvider
  }) || { supabaseUrl:'', supabaseAnonKey:'', vkAuthEnabled:buildVkAuthEnabled, vkAuthProvider:buildVkAuthProvider }
}
  ...
  runtimeState.promise = fetch('/api/auth/public-config', {
    ...
    .then(async response => {
      ...
      return setRuntimeSupabaseConfig({
        ...
        vkAuthEnabled:data.vkAuthEnabled,
        vkAuthProvider:data.vkAuthProvider
      })
    })
    .catch(() => null)
    .finally(() => {
