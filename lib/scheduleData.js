// AIanime v103: longer schedule cache to reduce Supabase egress.
import { hasSupabase, supabaseRequest } from '@/lib/supabaseServer'
import { isRestrictedAnimeSlug } from '@/lib/contentRestrictions'
...
const SCHEDULE_CACHE_TTL = Number(process.env.AIANIME_SCHEDULE_CACHE_TTL_MS || 1800000)

function readScheduleCache(key){
  const hit = scheduleCache.get(key)
  if(!hit) return null
  if(Date.now() - hit.at > SCHEDULE_CACHE_TTL){
...
function writeScheduleCache(key, value){
  scheduleCache.set(key, { at:Date.now(), value })
  if(scheduleCache.size > 16){
    const first = scheduleCache.keys().next().value
    if(first) scheduleCache.delete(first)
  }
  return value
}
...
function safePoster(url){
  ...
  if(!raw || !isUsableAnimePoster(raw)) return ''
  if(raw.startsWith('/')) return versionAnimePosterUrl(raw)
  return versionAnimePosterUrl(`/api/image?url=${encodeURIComponent(raw)}&fallback=${encodeURIComponent('/posters/magic2.svg')}`)
}
...
function normalizeScheduleRow(row, dynamicRestricted = new Set()){
  if(!row || !row.airing_at || !row.anime_slug) return null
  if(isRestrictedAnimeSlug(row.anime_slug || row.slug) || dynamicRestricted.has(String(row.anime_slug || row.slug || ''))) return null
  ...
  const poster = safePoster(row.poster_url || row.poster)
...
function emptyDays(now = new Date(), { timeZone = SCHEDULE_TIME_ZONE, rows = [], dynamicRestricted = new Set() } = {}){
  const weekStart = mondayOfWeekInTimeZone(now, timeZone)
  const todayKey = dateKeyInTimeZone(now, timeZone)
  const byKey = new Map()

  for(const row of rows){
    const item = normalizeScheduleRow(row, dynamicRestricted)
    if(!item) continue
...
async function readDynamicRestrictedSlugs(){
  if(!hasSupabase()) return new Set()
  try{
    const res = await supabaseRequest('anime?select=slug,status&status=eq.restricted_ru&limit=1400', { method:'GET', timeout:9000 })
    if(!res.ok) return new Set()
    const rows = await res.json().catch(() => [])
    return new Set((Array.isArray(rows) ? rows : []).map(row => String(row.slug || '')).filter(Boolean))
  }catch{
    return new Set()
  }
}
...
export async function getWeeklySchedule(options = {}){
  ...
  const cacheKey = `${timeZone}:${dateKeyInTimeZone(now, timeZone)}:${Number(options.limit || 300)}`
  const cached = readScheduleCache(cacheKey)
  if(cached) return cached

  const [result, dynamicRestricted] = await Promise.all([
    readScheduleRows({ now, timeZone, limit: options.limit }),
    readDynamicRestrictedSlugs(),
  ])
  const schedule = emptyDays(now, { timeZone, rows: result.rows, dynamicRestricted })
  const totalItems = schedule.days.reduce((sum, day) => sum + day.items.length, 0)

  return writeScheduleCache(cacheKey, {
    ...schedule,
    isReal: true,
