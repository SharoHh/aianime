...
import { isCatalogGuardEnabled, isUnsafeProductionCatalogCount, minProductionCatalogSize } from '@/lib/catalogGuard'
import { hasUsableAnimePoster } from '@/lib/animeQuality'
...
function animeStats(rows){
  ...
  return {
    ...
    posters: list.filter(row => hasUsableAnimePoster(row)).length,
    missingPosters: list.filter(row => !hasUsableAnimePoster(row)).length,
  }
}
...
export async function collectSiteHealth(){
  ...
  const warnings = []
  const minCatalogSize = minProductionCatalogSize()
  const catalogGuardEnabled = isCatalogGuardEnabled()
  const catalogDegraded = Boolean(catalogGuardEnabled && (!animeResult.ok || isUnsafeProductionCatalogCount(anime.total, 1200)))

  if(!supabaseConfigured) warnings.push('Supabase runtime выключен или не настроены env')
  if(catalogGuardEnabled && !supabaseConfigured) warnings.push('Production guard: Supabase обязателен, seed fallback заблокирован')
  if(catalogDegraded) warnings.push(`Production guard: каталог меньше ${minCatalogSize} или не прочитался`)
  ...
  const databaseOk = Boolean(supabaseConfigured && animeResult.ok && !catalogDegraded)
  ...
  return {
    ...
    env: {
      ...
      adminSecretConfigured: Boolean(process.env.ADMIN_SECRET || process.env.ADMIN_PASSWORD),
      adminExternalAuthTrusted: process.env.AIANIME_TRUST_NGINX_ADMIN_AUTH === '1',
    ...
    database: {
      ...
      minProductionCatalogSize: minCatalogSize,
      productionCatalogGuardEnabled: catalogGuardEnabled,
      catalogDegraded,
