...
export function buildYearSitemapEntries(anime = []){
  ...
  return [2026, 2025, 2024, 2023]
    .filter(year => (groups.get(year) || []).length > 0)
    .map(year => ({
      loc:sitemapAbsoluteUrl(`/anime-${year}`),
      lastmod:latestLastModified(groups.get(year) || []),
      changefreq:year >= 2026 ? 'daily' : 'weekly',
      priority:year >= 2026 ? .7 : .66,
    }))
}
