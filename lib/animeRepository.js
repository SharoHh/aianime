...
export async function getAnimeList({ limit = 300 } = {}){
  const safeLimit = Math.min(Math.max(Number(limit) || 300, 1), 1200)
  if(!hasSupabase()) return filterRestrictedAnime(seedFallbackList(safeLimit, 'supabase runtime disabled or env missing')).filter(isPublicListItem).slice(0, safeLimit)
  ...
  return withPending(cacheKey, async () => {
    ...
    try{
      ...
      if(!rows.length) return filterRestrictedAnime(seedFallbackList(safeLimit, 'supabase returned empty anime rows')).filter(isPublicListItem).slice(0, safeLimit)
      ...
      return filterRestrictedAnime(seedFallbackList(safeLimit, error?.message || 'supabase read failed')).filter(isPublicListItem).slice(0, safeLimit)
    }
  })
}
