...
export async function getEpisodesBySlug(slug, fallbackCount = 12){
  ...
  if(hasSupabase()){
    ...
    try{
      ...
      // Жёсткий limit=200 резал Фрирен примерно на первых 6 сериях при 30+ озвучках.
      const pageSize = 1000
      const maxRows = 12000
      const allRows = []
      ...
      if(allRows.length) return writeCache(episodeCache, cacheKey, allRows.map(mapDbEpisode))
    }catch(error){
