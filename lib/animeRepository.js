...
import { translateGenres, preferRussianDescription, makeRussianDescription } from '@/lib/ruContent'
...
const LIST_CACHE_TTL = Number(process.env.AIANIME_LIST_CACHE_TTL_MS || 300000)
const EPISODE_CACHE_TTL = Number(process.env.AIANIME_EPISODE_CACHE_TTL_MS || 180000)
...
export function mapDbAnime(row, index = 0){
  ...
  return {
    ...
    description: preferRussianDescription({ ...row, genres, title: row.title_ru || row.title, original_title: row.title_orig_kodik || row.original_title }),
    descriptionRu: row.description_ru || makeRussianDescription({ ...row, genres, title: row.title_ru || row.title, original_title: row.title_orig_kodik || row.original_title }),
    mood: [],
