...
function posterPath(value){
  ...
  if(!source) return ''
  try{
    return decodeURIComponent(new URL(source, 'https://aianime.local').pathname).toLowerCase()
  }catch{
...
function isPlaceholderPoster(url){
  ...
  const path = posterPath(source)
  if(!source || !path) return true
  if(isMissingPoster(source)) return true
  if(/^\/(?:posters|banners)\/[^/?#]+\.svg$/i.test(path)) return true
  if(/(?:^|\/)(?:placeholder|no[-_]?poster|default(?:-poster)?)(?:[._/-]|$)/i.test(path)) return true
  return false
}
