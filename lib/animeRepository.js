...
import {
  isPlaceholderPoster as isQualityPlaceholderPoster,
  isUsableAnimePoster,
  pickBestAnimePosterSource,
  isPublicReadyAnimeItem,
  isBrokenCatalogSlug as isQualityBrokenCatalogSlug,
  isLikelySeedOrSyntheticText
} from '@/lib/animeQuality'
...
const ANIME_SELECT = [
  ...
  'mal_id','shikimori_id','kodik_id','kodik_link','kodik_type','translation_title','translation_type','quality','kodik_quality','kodik_screenshots','screenshots','other_title','kodik_raw','raw','created_at','updated_at'
].join(',')
...
function readCachedAnimeList(limit, ttl = LIST_CACHE_TTL){
  ...
  return candidates[0].list.filter(isPublicListItem).slice(0, safeLimit)
}
...
function isLocalSvgPoster(url){
  return isQualityPlaceholderPoster(url) && /(?:^|\/|%2f)posters(?:\/|%2f)[^?#&"']+\.svg/i.test(String(url || '').toLowerCase())
}

function isPlaceholderPoster(url){
  return isQualityPlaceholderPoster(url)
}

function isBrokenCatalogSlug(slug){
  return isQualityBrokenCatalogSlug(slug)
}
...
function isLikelySyntheticBrokenText(item = {}){
  return isLikelySeedOrSyntheticText(item)
}
...
export function mapDbAnime(row, index = 0){
  ...
  return applyAnimeContentOverrides(row, {
    ...
    otherTitle: cleanPublicText(row.other_title) || null,
    titleOrigKodik: cleanPublicText(row.title_orig_kodik) || null,
    kodikTitle: cleanPublicText(row.kodik_raw?.title || row.kodik_raw?.material_data?.title || row.kodik_raw?.material_data?.anime_title) || null,
...
export async function getAnimeList({ limit = 300 } = {}){
  const safeLimit = Math.min(Math.max(Number(limit) || 300, 1), 1200)
  if(!hasSupabase()) return seedFallbackList(safeLimit, 'supabase runtime disabled or env missing')
  ...
  return withPending(cacheKey, async () => {
    ...
    try{
      ...
      if(!rows.length) return seedFallbackList(safeLimit, 'supabase returned empty anime rows')
      ...
      return seedFallbackList(safeLimit, error?.message || 'supabase read failed')
    }
  })
}
...
export async function getAnimeAdminList({ limit = 1000 } = {}){
  const safeLimit = Math.min(Math.max(Number(limit) || 1000, 1), 1200)
  if(!hasSupabase()) return seedAnime.map(mapSeedAnime).slice(0, safeLimit)
  ...
  try{
    ...
    assertSafeProductionCatalog(rows, safeLimit, 'anime admin list')
    return applyCommunityRatingStats(rows.map(mapDbAnime))
  }catch(error){
    console.warn('Supabase admin anime read failed:', error?.message || error)
    return []
  }
}
