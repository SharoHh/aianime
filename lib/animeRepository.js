// AIanime v95: server data cache/dedupe for faster page transitions.
...
const LIST_CACHE_TTL = Number(process.env.AIANIME_LIST_CACHE_TTL_MS || 900000)
const EPISODE_CACHE_TTL = Number(process.env.AIANIME_EPISODE_CACHE_TTL_MS || 600000)
const STALE_CACHE_TTL = Number(process.env.AIANIME_STALE_CACHE_TTL_MS || 3600000)
...
export async function getAnimeList({ limit = 300 } = {}){
  ...
  if(stale){
    refreshInBackground(cacheKey, async () => {
      let rows = []
      try{ rows = await readSupabaseTable('anime', safeLimit) }
      catch(firstError){ rows = await readSupabaseTable('anime_titles', safeLimit) }
      if(rows.length) writeCache(listCache, cacheKey, rows.map(mapDbAnime))
    })
    return stale
  }
