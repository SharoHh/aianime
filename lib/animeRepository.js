...
function isLocalSvgPoster(url){
  ...
  const values = [raw, decoded]
  return values.some(value => /^\/posters\/[^?#]+\.svg(?:[?#].*)?$/.test(value))
}
...
function isPlaceholderPoster(url){
  ...
  // /api/image?url=remote&fallback=/posters/*.svg остаётся валидным: смотрим только сам url,
  // а не fallback. Иначе реальные внешние постеры ошибочно вылетали бы из каталога.
  if(raw.startsWith('/api/image')){
    try{
      const parsed = new URL(raw, 'https://aianime.local')
      const source = parsed.searchParams.get('url') || ''
      return !source || isLocalSvgPoster(source) || isMissingPoster(source)
    }catch{
      return true
    }
  }
  if(isLocalSvgPoster(raw)) return true
  return /placeholder|no[-_]?poster|default/.test(raw)
}
