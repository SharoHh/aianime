...
import { filterRestrictedAnime, isRestrictedAnimeSlug } from '@/lib/contentRestrictions'
...
function isPublicListItem(item = {}){
  if(isRestrictedAnimeSlug(item?.slug || item?.anime_slug)) return false
...
export async function getAnimeBySlugFromRepo(slug){
  ...
  if(isRestrictedAnimeSlug(normalizedSlug)) return null
...
export function getSimilarFromList(item, list, limit = 6){
  ...
  const scored = list
    .filter(a => a.slug !== item.slug && !isRestrictedAnimeSlug(a.slug))
    .map(a => {
