...
function imageProxyMode(){
  return String(process.env.AIANIME_IMAGE_PROXY_MODE || process.env.NEXT_PUBLIC_AIANIME_IMAGE_PROXY_MODE || 'direct').toLowerCase()
}
...
async function readSupabaseTable(table, limit){
  ...
  const query = `${table}?select=${encodeURIComponent(ANIME_SELECT)}&order=rating.desc.nullslast&limit=${safeLimit}`
...
async function readAnimeBySlug(table, slug){
  ...
  const query = `${table}?select=${encodeURIComponent(ANIME_SELECT)}&slug=eq.${safeSlug}&limit=1`
  const res = await supabaseRequest(query, { method: 'GET', timeout: 7000 })
  if(!res.ok) throw new Error(`${table} slug: ${res.status} ${await res.text()}`)
  const rows = await res.json()
