...
const OPENAI_TIMEOUT_MS = Math.min(Math.max(Number(process.env.OPENAI_RECOMMEND_TIMEOUT_MS || process.env.AI_RECOMMEND_TIMEOUT_MS || 5500), 2500), 8000)
const GEMINI_TIMEOUT_MS = Math.min(Math.max(Number(process.env.GEMINI_RECOMMEND_TIMEOUT_MS || process.env.AI_RECOMMEND_TIMEOUT_MS || 5500), 2500), 8000)
const OPENAI_CANDIDATE_LIMIT = Math.min(Math.max(Number(process.env.OPENAI_RECOMMEND_CANDIDATES || process.env.AI_RECOMMEND_CANDIDATES || 14), 10), 18)
const EXTERNAL_AI_TIMEOUT_MS = Math.min(Math.max(Number(process.env.AI_RECOMMEND_TIMEOUT_MS || process.env.EXTERNAL_AI_TIMEOUT_MS || 8000), 2500), 8000)
...
function franchiseSpecificReason(item = {}, query = ''){
  const title = cleanTitle(item)
  const original = String(item?.originalTitle || item?.englishTitle || '').trim()
  const blob = `${title} ${original} ${item?.slug || ''}`.toLowerCase().replace(/ё/g, 'е')
  const tone = detectHumanQueryTone(query)

  if(tone.dark){
    if(blob.includes('shingeki') || blob.includes('атака титанов')) return 'Здесь не просто мрак ради мрака: герой ломается об потери, страх и войну, но всё равно прёт вперёд.'
    if(blob.includes('monster') || blob.includes('монстр')) return 'Медленный психологический кошмар про человека, который тащит вину за своё решение и идёт до конца.'
    if(blob.includes('steins') || blob.includes('врата штейна')) return 'Хороший выбор, если хочется героя, которого раз за разом давят последствия, но он всё равно пытается спасти своих.'
    if(blob.includes('tokyo ghoul') || blob.includes('токийский гуль')) return 'Попадает в запрос через потерю себя: герой буквально становится другим и пытается выжить в очень тёмном мире.'
    if(blob.includes('berserk') || blob.includes('берсерк')) return 'Это самый тяжёлый вариант: герой собран из боли, ярости и выживания, но именно поэтому он тащит.'
    if(blob.includes('chainsaw') || blob.includes('бензопил')) return 'Мрачный, грязный и нервный экшен, где герой выглядит простым, но внутри постоянно живёт на обломках.'
    if(blob.includes('inuyashiki') || blob.includes('инуяшики')) return 'Работает как история про сломанную жизнь и внезапную силу, которую герой использует не ради красивой позы.'
    if(blob.includes('vinland') || blob.includes('винланд')) return 'Здесь надлом не показной: персонаж проходит через насилие, вину и попытку заново понять, ради чего жить.'
    if(blob.includes('re:zero') || blob.includes('re zero') || blob.includes('жизнь с нуля')) return 'Подходит, если нужен герой, которого многократно ломают страхом и смертью, но он всё равно возвращается.'
    if(blob.includes('parasyte') || blob.includes('паразит')) return 'История постепенно ломает героя через страх, отчуждение и необходимость принимать нечеловеческие решения.'
    if(blob.includes('made in abyss') || blob.includes('созданный в бездне')) return 'За красивой оболочкой тут очень жёсткое путешествие, где надежда постоянно сталкивается с болью и потерями.'
  }
...
function humanReasonForItem(item = {}, query = ''){
  ...
  const franchiseReason = franchiseSpecificReason(item, query)
  if(franchiseReason) return franchiseReason
...
function franchiseBlob(item = {}){
  return [
    ...
    item?.originalTitle,
    item?.englishTitle
  ].map(value => String(value || '').toLowerCase().replace(/ё/g, 'е')).join(' ')
}

function knownFranchiseKey(item = {}){
  const blob = franchiseBlob(item)
  const known = [
    ['aot', ['shingeki no kyojin', 'attack on titan', 'атака титанов']],
    ['tokyo-ghoul', ['tokyo ghoul', 'toukyou kushu', 'токийский гуль']],
    ['mushoku-tensei', ['mushoku tensei', 'реинкарнация безработного']],
    ['no-game-no-life', ['no game no life', 'nogemu noraifu', 'нет игры', 'нет жизни']],
    ['re-zero', ['re:zero', 're zero', 're-zero', 'жизнь с нуля']],
    ['one-piece', ['one piece', 'ван-пис', 'ван пис']],
    ['naruto', ['naruto', 'наруто']],
    ['bleach', ['bleach', 'блич']],
    ['demon-slayer', ['kimetsu no yaiba', 'demon slayer', 'клинок рассекающий демонов']],
    ['jojo', ['jojo', 'джоджо']],
    ['vinland-saga', ['vinland saga', 'сага о винланде']],
    ['evangelion', ['evangelion', 'евангелион']],
    ...
    ['fullmetal-alchemist', ['fullmetal alchemist', 'hagane no renkinjutsushi', 'стальной алхимик']]
  ]
  for(const [key, needles] of known){
    if(needles.some(needle => blob.includes(needle))) return key
  }
  return ''
}

function itemFranchiseKey(item = {}){
  const known = knownFranchiseKey(item)
  if(known) return known

  let key = duplicateCandidateKey(item)
  key = String(key || '').toLowerCase()
    .replace(/\btv\b|\bova\b|\bona\b|\bmovie\b|\bfilm\b|\bspecials?\b|\bseason\b|\bpart\b|\bfinal\b|\bkanketsu\b|\bhen\b/g, '')
    .replace(/тв|ова|она|фильм|спецвыпуски|спешл|сезон|часть|финал|полнометражный/g, '')
    .replace(/[0-9]+(st|nd|rd|th)?/g, '')
    .replace(/[^a-zа-я]+/g, '')
    .trim()
  return key || String(item?.slug || '').toLowerCase()
}

function entityTargetFranchiseKeys(entityHints = []){
  const keys = new Set()
  for(const hint of entityHints || []){
    for(const target of hint.titleTargets || []){
      const probe = { title: target, originalTitle: target, slug: target }
      const key = knownFranchiseKey(probe) || normalizeAiText(target)
      if(key) keys.add(key)
    }
  }
  return keys
...
function applyFranchiseDiversity(items = [], query = '', limit = 8, entityHints = []){
  const protectedKeys = entityTargetFranchiseKeys(entityHints.length ? entityHints : getQueryEntityHints(query))
  const broadQuery = protectedKeys.size === 0
  const source = items.filter(item => item?.slug && !isSeedCatalogItem(item))
  const selected = []
  const used = new Set()
  const counts = new Map()

  const pushPass = (capForItem) => {
    for(const item of source){
      const slug = String(item.slug)
      if(used.has(slug)) continue
      const key = itemFranchiseKey(item)
      const current = counts.get(key) || 0
      const cap = capForItem(item, key, current)
  ...
  // For vague mood requests, one franchise should not occupy the whole page.
  // First pass: one title per franchise. This fixes cases where Gemini returns
  // six Attack on Titan seasons for a broad “dark broken hero” query.
  pushPass((item, key) => protectedKeys.has(key) ? 3 : 1)

  // Second pass: allow a second title from the same franchise only after
  // the page already has a diverse core. Exact character/title queries keep
  // a larger cap, e.g. “Канеки” may reasonably show Tokyo Ghoul entries.
  if(selected.length < limit){
    pushPass((item, key) => protectedKeys.has(key) ? 3 : 2)
  }

  // Final emergency fill. For broad requests we still keep a hard cap of 2
  // unless there are simply not enough unique franchises in the candidates.
  if(selected.length < limit){
    const uniqueFranchises = new Set(source.map(itemFranchiseKey)).size
    const emergencyCap = broadQuery && uniqueFranchises >= Math.min(5, limit) ? 2 : 4
    pushPass((item, key) => protectedKeys.has(key) ? 4 : emergencyCap)
  }
...
function makeFallbackResults(candidates, query, limit){
  return applyFranchiseDiversity(candidates, query, limit)
    .map(item => ({
      ...item,
...
function buildCandidatePool(list, query, options = {}){
  ...
  return applyFranchiseDiversity(rows, query, limit, entityHints)
}
...
function mergeModelResults(parsed, candidatePool, query, limit, provider = 'ai'){
  ...
  const usedAfterModel = new Set(selected.map(item => item.slug))
  const filler = candidatePool.filter(item => item?.slug && !usedAfterModel.has(item.slug)).map(item => ({
    ...item,
    match: Math.min(88, Math.max(60, Math.round(item.match || 58 + Number(item.aiScore || 0) / 22))),
    reason: cleanModelReason(item.reason, item, query),
    aiSource: 'local-diversity'
  }))

  // Важно: финальную франшизную чистку делаем один раз по ОБЩЕМУ списку.
  // Раньше fill-логика запускала diversity отдельно и могла снова добавить
  // 3-4 сезона одной франшизы после Gemini/backup-ответа.
  const diversified = applyFranchiseDiversity([...selected, ...filler], query, limit)
...
async function recommendViaExternalBackend(payload, candidatePool, query, limit, model, provider){
  ...
  try{
    ...
    // Если AI-VPS вернул backup из-за 503/high demand/таймаута, не считаем это
    // полноценным AI-ответом. Пусть основной сайт покажет свой быстрый подбор,
    // уже прошедший строгую чистку дублей франшиз.
    if(parsed?.source === 'external-local' && parsed?.meta?.ok === false){
      return {
        configured:true,
        ok:false,
        status: parsed?.meta?.status || 503,
        error: truncateText(parsed?.meta?.error || 'external local fallback', 260)
      }
    }
...
export async function recommendWithOpenAI(list, query = '', options = {}){
  ...
  const payload = {
    ...
    rules: [
      ...
      'Для широкого вайб-запроса не забивай выдачу сезонами одной франшизы: максимум 1-2 тайтла одной серии, дальше разнообразь.'
    ],
    candidates
  }
