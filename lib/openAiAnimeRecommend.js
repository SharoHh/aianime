...
function buildCandidatePool(list, query, options = {}){
  ...
  const localLimit = Math.max(OPENAI_CANDIDATE_LIMIT, Number(options.limit || 12) * 6)
  const local = recommendAnime(safeList, query, { ...options, limit: localLimit })
  const seen = new Set(local.map(item => item.slug))

  const scored = safeList
    .filter(item => item?.slug && item.slug !== baseSlug && !seen.has(item.slug))
    .map(item => ({ ...item, aiScore: scoreAiItem(item, query), reason: explainAiMatch(item, query) }))
    .filter(item => Number(item.aiScore || 0) > 0)
    .sort((a,b) => Number(b.aiScore || 0) - Number(a.aiScore || 0))
    .slice(0, Math.max(0, OPENAI_CANDIDATE_LIMIT - local.length))

  return [...local, ...scored].filter(item => item?.slug).slice(0, OPENAI_CANDIDATE_LIMIT)
}
