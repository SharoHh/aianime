...
function mergeModelResults(parsed, candidatePool, query, limit, provider = 'ai'){
  ...
  const diversified = applyFranchiseDiversity(selected, query, limit)
  if(diversified.length < limit){
    const used = new Set(diversified.map(item => item.slug))
    const filler = candidatePool.filter(item => item?.slug && !used.has(item.slug)).map(item => ({
      ...item,
      match: Math.min(88, Math.max(60, Math.round(item.match || 58 + Number(item.aiScore || 0) / 22))),
      reason: cleanModelReason(item.reason, item, query),
      aiSource: 'local-diversity'
    }))
    diversified.push(...applyFranchiseDiversity(filler, query, limit - diversified.length))
  }
