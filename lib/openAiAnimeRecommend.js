...
function buildCandidatePool(list, query, options = {}){
  ...
  const limit = Math.max(OPENAI_CANDIDATE_LIMIT, Number(options.limit || 12) * 8)
  const intent = getQueryIntent(query)
  const bySlug = new Map()

  function add(items = []){
    for(const raw of items){
      if(!raw?.slug || raw.slug === baseSlug || bySlug.has(raw.slug)) continue
      const item = decorateCandidate(raw, query, intent)
      bySlug.set(item.slug, item)
      if(bySlug.size >= limit) break
    }
  }

  // 1) локально самые подходящие, но не даём строгому фильтру задушить выборку до 1 тайтла.
  add(recommendAnime(safeList, query, { ...options, limit }))

  // 2) точные жанры пользователя: для запроса «романтика про школу» сюда попадут все романтические/школьные тайтлы.
  if(intent.wantedGenres.length && bySlug.size < limit){
    add(
      safeList
        .filter(item => item.slug !== baseSlug && hasAnyGenre(item, intent.wantedGenres))
        .map(item => decorateCandidate(item, query, intent))
        .sort((a,b) => Number(b.aiScore || 0) - Number(a.aiScore || 0))
    )
  }

  // 3) соседние жанры по вайбу, чтобы AI мог подобрать больше 8 вариантов, а не работать как тупой фильтр.
  if(intent.wantedGenres.length && bySlug.size < Math.min(limit, 48)){
    const related = relatedGenresForCandidatePool(intent.wantedGenres)
    add(
      safeList
        .filter(item => item.slug !== baseSlug && hasAnyGenre(item, related))
        .map(item => decorateCandidate(item, query, intent))
        .sort((a,b) => Number(b.aiScore || 0) - Number(a.aiScore || 0))
    )
  }

  // 4) общий мягкий добор по scoreAiItem, чтобы внешний OpenAI всегда получил нормальную витрину кандидатов.
  if(bySlug.size < Math.min(limit, 64)){
    add(
      safeList
        .filter(item => item.slug !== baseSlug)
        .map(item => decorateCandidate(item, query, intent))
        .filter(item => Number(item.aiScore || 0) > -120)
        .sort((a,b) => Number(b.aiScore || 0) - Number(a.aiScore || 0))
    )
  }
  ...
  // 5) страховка: если данных по жанру мало, не отдаём OpenAI одну карточку — пусть выберет из лучших похожих.
  if(rows.length < Math.min(Number(options.limit || 12), 8)){
    const seen = new Set(rows.map(item => item.slug))
    const extras = safeList
      .filter(item => item.slug !== baseSlug && !seen.has(item.slug))
      .map(item => decorateCandidate(item, query, intent))
      .sort((a,b) => Number(b.aiScore || 0) - Number(a.aiScore || 0))
      .slice(0, Math.min(limit - rows.length, 24))
    rows.push(...extras)
  }
...
async function recommendViaExternalBackend(payload, candidatePool, query, limit, model){
  ...
  try{
    ...
    return {
      ...
      results,
      meta: parsed?.openai || parsed?.meta || null
    }
  }catch(error){
    clearTimeout(timeout)
    return {
...
async function recommendDirectOpenAI(payload, candidatePool, query, limit, localFallback, apiKey, model){
  ...
  try{
    ...
    return {
      ...
      results,
      openai: { enabled:true, ok:true, usage:data?.usage || null }
    }
  }catch(error){
    clearTimeout(timeout)
    return {
...
export async function recommendWithOpenAI(list, query = '', options = {}){
  ...
  if(external.configured && external.ok){
    return {
      ...
      openai: { enabled:true, ok:true, via:'external', endpointConfigured:true, meta:external.meta || null }
    }
  }
