  ...
  try{
    ...
    if(!res.ok){
      ...
      return {
        source: 'local',
        model,
        summary: 'Gemini сейчас не ответил, показан локальный запасной подбор.',
        results: localFallback,
    ...
    return {
      source: 'local',
      model,
      summary: 'Gemini не ответил по таймауту/ошибке, показан локальный запасной подбор.',
      results: localFallback,
...
export async function recommendWithOpenAI(list, query = '', options = {}){
  ...
  if(provider === 'gemini'){
    ...
    if(!geminiKey){
      return localPayload(
        external.configured
          ? 'Внешний AI-backend не ответил, Gemini API key на основном сервере не задан, показан локальный запасной подбор.'
