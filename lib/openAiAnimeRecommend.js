...
const OPENAI_CANDIDATE_LIMIT = Math.min(Math.max(Number(process.env.OPENAI_RECOMMEND_CANDIDATES || process.env.AI_RECOMMEND_CANDIDATES || 24), 16), 32)
const EXTERNAL_AI_TIMEOUT_MS = Math.min(Number(process.env.AI_RECOMMEND_TIMEOUT_MS || process.env.EXTERNAL_AI_TIMEOUT_MS || 4500), 6500)
...
function compactCandidate(item, query, index){
  const genres = Array.isArray(item?.genres) ? item.genres.filter(Boolean).slice(0, 5) : []
  return {
    n: index + 1,
    ...
    localReason: truncateText(item?.reason || explainAiMatch(item, query), 80)
  }
}
...
function buildCandidatePool(list, query, options = {}){
  ...
  const limit = Math.min(
    Math.max(OPENAI_CANDIDATE_LIMIT, requestedLimit * 3, 20),
    32
  )
  ...
  add(prepared(recommendAnime(safeList, query, { ...options, limit: Math.min(limit, 30) }), 60), Math.min(limit, 32))
...
export async function recommendWithOpenAI(list, query = '', options = {}){
  ...
  const payload = {
    ...
    ].map(hint => ({ id: hint.id, note: hint.note, target_titles: hint.titleTargets, text_targets: hint.textTargets, genres: hint.genres })),
    rules: [
      'Выбирай только из candidates, slug не придумывай.',
      'До 8 результатов. Точное название из запроса всегда первым.',
      'Смотри смысл, жанры, вайб и отрицания.',
      'Если query_hints не пустой, это важная подсказка: персонаж, тема или кривое слово должны направлять подбор.',
      'Если точного совпадения в candidates нет, честно подбери ближайшие варианты и не выдумывай факты про персонажей.',
      'reason коротко по-русски, 1 фраза.'
    ],
    candidates
  }
