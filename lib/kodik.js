...
function animeKindText(anime = {}){
  return normalizeTitle([anime?.kind, anime?.type, anime?.kodikType, anime?.slug, anime?.title, anime?.title_ru, anime?.original_title, anime?.originalTitle, anime?.englishTitle].filter(Boolean).join(' '))
}

function hasSeasonMarkerText(text){
  return /(?:season|сезон|tv|тв)\s*\d|\d+(?:st|nd|rd|th)\s+season|s\d+/.test(text)
}

function hasSpecialMarkerText(text){
  return /ova|ona|special|спешл|спец|kuinaki|sentaku|regrets|reze|movie|film|фильм|lost\s+girls|no\s+regrets|выбор\s+без\s+сожалений/.test(text)
}
...
function expectedEpisodeMismatch(material, anime = {}){
  ...
  // OVA/спешлы/фильмы с 1–6 сериями не должны получать TV-сезон на 12/25/148 серий.
  if(expected <= 6 && actual > Math.max(expected + 4, expected * 2)) return true
  if(specialLike && expected <= 12 && actual > expected + 6) return true

  // Страницы конкретных сезонов не должны получать соседний сезон с другим количеством серий.
