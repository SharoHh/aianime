...
export function scoreAiItem(item, query = '', context = {}){
  ...
  if(intent.strongGenreRequest && intent.wantedGenres.length && !wantedOverlap.length) score -= 320

  if(intent.wantedGenres.includes('Романтика')){
    if(genres.includes('Романтика')) score += 80
    if(!genres.includes('Романтика')) score -= 220
    if(['Экшен','Ужасы','Триллер'].some(g => genres.includes(g)) && !genres.includes('Романтика')) score -= 120
  }
...
export function isAiItemRelevant(item, query = ''){
  ...
  if(intent.strongGenreRequest && intent.wantedGenres.length && !hasAnyWantedGenre(item, intent.wantedGenres)) return false
  if(intent.wantedGenres.includes('Романтика') && !itemHasGenre(item, ['Романтика'])) return false
  if(intent.light || intent.noHeavy){
    if(itemHeavySignals(item) > 45 && !itemHasGenre(item, LIGHT_SAFE_GENRES)) return false
  }
  return true
}
...
export function getCatalogHint(query = ''){
  ...
  if(intent.strongGenreRequest && intent.wantedGenres.length) return `Жёстко держимся жанра: ${intent.wantedGenres.join(', ')}. Тайтлы без этого жанра не поднимаем в выдачу.`
