...
const LAT_TO_CYR_HINTS = [
  ...
  ['chainsaw', 'человек бензопила'], ['one piece', 'ван пис'], ['onepiece', 'ван пис one piece'], ['ванпис', 'one piece ван пис'], ['ван пис', 'one piece'], ['naruto', 'наруто'], ['bleach', 'блич'],
...
function hasPlaceholderPoster(item = {}){
  const poster = String(item.poster || item.poster_url || '').toLowerCase()
  return !poster || /\/posters\/magic|placeholder|no[-_]?poster|default/.test(poster)
}

function catalogQualityBoost(item = {}){
  let score = 0
  if(!hasPlaceholderPoster(item)) score += 180
  if(item.titleRu || /[А-Яа-яЁё]/.test(String(item.title || item.displayTitle || ''))) score += 45
  if(String(item.description || item.descriptionRu || '').trim().length >= 90) score += 35
  if(item.kodikId || item.kodikLink || item.translationTitle) score += 25
  return score
}
...
export function filterAndSortAnime(items = [], query = '', filters = {}, sort = 'popular'){
  ...
  scored.sort((a, b) => {
    ...
    const qualityDiff = catalogQualityBoost(b.item) - catalogQualityBoost(a.item)
    if(!text && qualityDiff) return qualityDiff
