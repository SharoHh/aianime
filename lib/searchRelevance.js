...
const GENRE_ALIASES = [
  ['Комедия', ['комедия', 'смешное', 'смешной', 'веселое', 'весёлое', 'ржач', 'юмор']],
  ['Повседневность', ['повседневность', 'лайф', 'slice', 'slice of life', 'уютное', 'спокойное']],
  ['Романтика', ['романтика', 'романтическое', 'любовь', 'отношения', 'милое']],
  ['Экшен', ['экшен', 'боевик', 'драки', 'битвы', 'сражения', 'динамичное']],
...
function genreHitsFromQuery(query = ''){
  ...
  for(const [genre, aliases] of GENRE_ALIASES){
    if(aliases.some(alias => q.includes(normalizeSearchText(alias)))) hits.push(genre)
  }
  return hits
}
...
export function getQueryIntent(query = ''){
  ...
  for(const [genre, aliases] of GENRE_ALIASES){
    for(const alias of aliases){
      ...
      if(q.includes(`без ${a}`) || q.includes(`не ${a}`) || q.includes(`не хочу ${a}`)){
  ...
  return {
    ...
    wantedGenres: Array.from(new Set(wantedGenres)),
...
export function scoreCatalogItem(item, query = ''){
  ...
  for(const genre of intent.wantedGenres){
    if(genres.includes(genre)) score += 32
  }
...
export function scoreAiItem(item, query = '', context = {}){
  const intent = getQueryIntent(query)
  const genres = Array.isArray(item.genres) ? item.genres : []
  let score = Number(item.score || item.rating || 0) * 7 + Number(item.popularity || 0) / 8
  score += scoreCatalogItem(item, query) * 1.2

  if(!intent.raw) score += Number(item.score || 0)

  for(const genre of intent.wantedGenres){
    if(genres.includes(genre)) score += 36
  }
...
export function explainAiMatch(item, query = '', context = {}){
  ...
  if(intent.light){
    ...
    if(overlap.length) parts.push(`совпадает по жанрам: ${overlap.join(', ')}`)
  }
...
export function getCatalogHint(query = ''){
  ...
  if(intent.light) return 'Для лёгких запросов мы понижаем тяжёлую драму, триллеры и жёсткий экшен.'
